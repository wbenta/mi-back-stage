  ...
  <div class="mittopnav-container">
    <el-form :inline="true" :model="formInline" class="demo-form-inline">
      <el-form-item label="分类">
        <el-input v-model="formInline.cate" placeholder="分类"></el-input>
      </el-form-item>
...
<script>
import { getcate, insertcate, updatecate, deletecate } from '@/api/getcate.js'
export default {
  data() {
    return {
      // el-from
  ...
  methods: {
    ...
    async onSubmit(type) {
      ...
      if (type === 1) {
        ...
        const { data: res } = await insertcate(
          this.formInline.cate,
          this.formInline.src
        )
        console.log(res)
        if (res.status === 1) {
    ...
    handleSave(item) {
      this.$confirm('此操作将永久修改信息, 是否继续?', '提示', {
        ...
        .then(async () => {
          console.log(item)
          const { data: res } = await updatecate(item.cate, item.src, item.id)
          console.log(res)
          item.disabled = true
          this.$message({
            type: 'success',
